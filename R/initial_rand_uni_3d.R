#' @title initial_rand_uni_3d
#'
#' @description A function to generate an initial population as a 3D array. Each element will be
...
#' }
#'
#' @return an array representing a 3D population of genotypes
initial_rand_uni_3d <- function(self, par){
  return(
    array(
      runif(
        par$geno_length * par$population_size * 2,
        min = par$initial_min,
        max = par$initial_max
      ),
      dim = c(par$geno_length, 2, par$population_size)
    )
  )
}
